...
public final class Interval1D {
	...
	public int sizeOfIntersectionWith(Interval1D that) {
		if ((this.includedTo() < that.includedFrom()) || (this.includedFrom() > that.includedTo())) {
			...
			int min = Math.max(this.includedFrom(), that.includedFrom());
			int max = Math.min(this.includedTo(), that.includedTo());
	...
	public Interval1D boundingUnion(Interval1D that) {
	    int min = Math.min(this.includedFrom(), that.includedFrom());
        int max = Math.max(this.includedTo(), that.includedTo());
